...
class FR3CuroboPlanner(MotionPlannerBase):
    ...
    # Isaac Lab uses FR3 joint names, while CuroboPlannerCfg.franka_config()
    # loads a Panda URDF. The joint values have the same order, so translate
    # names only at the Isaac Lab/cuRobo boundary.
    FR3_TO_PANDA_JOINT_NAMES: dict[str, str] = {
        ...
        "fr3_finger_joint1": "panda_finger_joint1",
        "fr3_finger_joint2": "panda_finger_joint2",
    }

    PANDA_TO_FR3_JOINT_NAMES: dict[str, str] = {
        panda_name: fr3_name
        for fr3_name, panda_name in FR3_TO_PANDA_JOINT_NAMES.items()
    }
    ...
    def __init__(
        ...
        motion_gen_config: MotionGenConfig = MotionGenConfig.load_from_robot_config(
            ...
            # gradient_ik_file = "gradient_ik_autotune.yml",
        )

        # Create motion generator
        self.motion_gen: MotionGen = MotionGen(motion_gen_config)
        # self.motion_gen.rollout_fn.primitive_collision_constraint.use_sweep = True
        # print('enabled', self.motion_gen.rollout_fn.primitive_collision_constraint.enabled) # True by default
        # print('use_sweep', self.motion_gen.rollout_fn.primitive_collision_constraint.use_sweep) # False by default
        # print('sum_distance', self.motion_gen.rollout_fn.primitive_collision_constraint.sum_distance) # True
        # print('classify', self.motion_gen.rollout_fn.primitive_collision_constraint.classify) # True
        # print('weight', self.motion_gen.rollout_fn.primitive_collision_constraint.weight) # 2000. by default
        # print('activation_distance', self.motion_gen.rollout_fn.primitive_collision_constraint.activation_distance) # 0. by default\
        # print('return_loss', self.motion_gen.rollout_fn.primitive_collision_constraint.return_loss) # False

        self.collision_detector = self.motion_gen.rollout_fn.primitive_collision_constraint.forward
        # self.collision_dis_checker = self.motion_gen.rollout_fn.primitive_collision_constraint.world_coll_checker.get_sphere_distance
        self.collision_dis_checker = self.motion_gen.rollout_fn.primitive_collision_constraint.world_coll_checker.get_swept_sphere_collision

        # print("Collision detector function:", self.collision_detector) # worldmeshcollision
        ...
        # # Debug controls for world-pose diff logging
        # self._world_update_counter: int = 0
        # # Log pose diffs only every N update_world() calls (to avoid spam)
        # self._debug_every_n_world_updates: int = int(getattr(self.config, "debug_every_n_world_updates", 1))
  
        # # -------------------------------------------------------------------------
        # # DEBUG: cuboid dump controls (pose + size/scale)
        # # -------------------------------------------------------------------------
        # # If config provides debug_cuboid_names, use it. Otherwise default to obstacle_1..3.
        # self._debug_cuboid_names: list[str] = list(
        #     getattr(self.config, "debug_cuboid_names", ["obstacle_1", "obstacle_2", "obstacle_3"])
        # )
        # # Dump cuboid details every N world updates (default: same cadence as pose diff)
        # self._debug_every_n_cuboid_dumps: int = int(
        #     getattr(self.config, "debug_every_n_cuboid_dumps", self._debug_every_n_world_updates)
        # )
        # self._debug_cuboid_always: bool = bool(getattr(self.config, "debug_cuboid_always", False))

        # pos_diff, rot_diff = self.debug_check_robot_root_matches_robot_prim(
        #         env=self.env, robot=self.robot, usd_helper=self.usd_helper, env_id=0
        #     )
    ...
    # =====================================================================================
    # DEBUG HELPERS: CUBOID DUMP (cuRobo vs IsaacLab/USD)
    # =====================================================================================

    def _find_curobo_primitive(self, world_model, object_path: str, primitive_type: str):
        """Find a cuRobo primitive (mesh/cuboid/...) by robust path matching."""
        try:
            if isinstance(world_model, list):
                if len(world_model) <= self.env_id:
                    return None
                world_model = world_model[self.env_id]
            prim_list = getattr(world_model, primitive_type, None)
            if not prim_list:
                return None
            for prim in prim_list:
                if not getattr(prim, "name", None):
                    continue
                prim_name = str(prim.name)
                if object_path == prim_name or object_path in prim_name or prim_name in object_path:
                    return prim
        except Exception:
            return None
        return None

    def _get_usd_scale_and_size(self, prim_path: str) -> dict[str, Any]:
        """Best-effort extraction of USD scale / size for cuboid-ish prims.

        Notes:
        - This is intentionally robust and may return partial info.
        - For UsdGeom.Cube: attribute "size" exists (edge length in local space),
          and Xform scale may be applied via xformOp:scale.
        """
        out: dict[str, Any] = {"prim_path": prim_path, "usd_valid": False}
        try:
            ...
            # Common xform scale op
            scale_attr = prim.GetAttribute("xformOp:scale")
            if scale_attr and scale_attr.IsValid():
                try:
                    out["xform_scale"] = list(scale_attr.Get())
                except Exception:
                    out["xform_scale"] = None
            else:
                out["xform_scale"] = None

            # UsdGeom.Cube "size" attribute (local edge length)
            size_attr = prim.GetAttribute("size")
            if size_attr and size_attr.IsValid():
                try:
                    out["cube_size_attr"] = float(size_attr.Get())
                except Exception:
                    out["cube_size_attr"] = None
            else:
                out["cube_size_attr"] = None

            # Some assets use "extent" (min/max) locally
            extent_attr = prim.GetAttribute("extent")
            if extent_attr and extent_attr.IsValid():
                try:
                    extent_val = extent_attr.Get()  # typically [(minx,miny,minz),(maxx,maxy,maxz)]
                    out["extent_attr"] = [list(extent_val[0]), list(extent_val[1])]
                except Exception:
                    out["extent_attr"] = None
            else:
                out["extent_attr"] = None
    ...
    def _debug_dump_cuboid_consistency(
        self,
        object_name: str,
        object_path: str,
        lab_pos: torch.Tensor,
        lab_quat: torch.Tensor,
    ) -> None:
        """Dump cuRobo cuboid and USD/Lab pose/size info side-by-side.

        This is the key "world model verification" step:
          - cuRobo world_model.cuboid pose + dims
          - IsaacLab env-local pose we just used for update
          - USD prim scale/size attributes (best-effort)
        """
        # cuRobo world model primitive
        world_model = self.motion_gen.world_coll_checker.world_model
        cu_cub = self._find_curobo_primitive(world_model, object_path, "cuboid")

        # Always include the "pose we intended to set" (Lab env-local)
        lab_pos_l = lab_pos.detach().to("cpu")
        lab_quat_l = lab_quat.detach().to("cpu")

        msg_lines: list[str] = []
        msg_lines.append(f"[CUBOID_DUMP] {object_name}")
        msg_lines.append(f"  path: {object_path}")
        msg_lines.append(f"  lab_pose(env-local): pos={lab_pos_l.tolist()} quat(wxyz)={lab_quat_l.tolist()}")

        # cuRobo cuboid info (pose + dims/scale)
        if cu_cub is None:
            msg_lines.append("  curobo_cuboid: NOT FOUND in world_model.cuboid (mapping mismatch?)")
        else:
            # pose list in cuRobo world model is typically [x,y,z,w,x,y,z]
            cu_pose_list = getattr(cu_cub, "pose", None)
            cu_name = str(getattr(cu_cub, "name", ""))
            msg_lines.append(f"  curobo_cuboid.name: {cu_name}")
            msg_lines.append(f"  curobo_cuboid.pose(list): {cu_pose_list}")

            # dims field name differs across versions; try a few
            cu_dims = None
            for k in ["dims", "dimension", "dimensions", "size", "half_dims", "half_extents", "extents"]:
                if hasattr(cu_cub, k):
                    try:
                        cu_dims = getattr(cu_cub, k)
                        break
                    except Exception:
                        pass
            if cu_dims is None:
                msg_lines.append("  curobo_cuboid.dims: <unavailable>")
            else:
                # make it printable
                try:
                    if isinstance(cu_dims, torch.Tensor):
                        msg_lines.append(f"  curobo_cuboid.dims: {cu_dims.detach().cpu().tolist()}")
                    else:
                        msg_lines.append(f"  curobo_cuboid.dims: {list(cu_dims) if hasattr(cu_dims,'__iter__') else cu_dims}")
                except Exception:
                    msg_lines.append(f"  curobo_cuboid.dims: {cu_dims}")

        # USD best-effort size/scale (this often reveals "scale not applied" bugs)
        usd_info = self._get_usd_scale_and_size(object_path)
        msg_lines.append(
            "  usd: "
            f"valid={usd_info.get('usd_valid', False)} "
            f"xform_scale={usd_info.get('xform_scale', None)} "
            f"cube_size_attr={usd_info.get('cube_size_attr', None)} "
            f"extent_attr={usd_info.get('extent_attr', None)}"
        )
        if "error" in usd_info:
            msg_lines.append(f"  usd.error: {usd_info['error']}")

        # Emit as one block to keep logs readable
        self.logger.debug("\n".join(msg_lines))
    ...
    def _sync_object_poses_with_isaaclab(self) -> None:
        ...
        # # Decide whether to print pose diffs this call
        # do_pose_diff_log = (
        #     bool(self.config.debug_planner)
        #     and self._debug_every_n_world_updates > 0
        #     and (self._world_update_counter % self._debug_every_n_world_updates == 0)
        # )

        # # Thresholds (tune if needed)
        # pos_warn_m = float(getattr(self.config, "debug_pose_diff_pos_warn_m", 0.02))   # 2 cm
        # rot_warn_deg = float(getattr(self.config, "debug_pose_diff_rot_warn_deg", 5.0))  # 5 deg

        # # Cuboid dump cadence
        # do_cuboid_dump = (
        #     bool(self.config.debug_planner)
        #     and (self._debug_cuboid_always or self._debug_every_n_cuboid_dumps > 0)
        #     and (self._debug_cuboid_always or (self._world_update_counter % self._debug_every_n_cuboid_dumps == 0))
        # )
        ...
        for object_name, object_path in object_mappings.items():
            ...
            p_w = obj.data.root_pos_w[self.env_id, :3]
            q_w = obj.data.root_quat_w[self.env_id, :4]  # (w,x,y,z)

            # Convert WORLD -> RobotPrim frame (matches reference_prim_path used at static-world init)
            p_rel, q_rel = self._world_to_robotprim_pose(p_w, q_w, env_id=self.env_id)

            current_pos_raw = p_rel
            current_quat_raw = q_rel
            ...
            if self._update_object_in_world_model(world_model, object_name, object_path, pose_list):
                ...
                # # -------------------------------------------------------------------------
                # # DEBUG: Dump cuboid pose + size consistency for obstacle_1..3 (or configured names)
                # # -------------------------------------------------------------------------
                # if do_cuboid_dump:
                #     try:
                #         # Match by substring (case-insensitive) so you can pass ["Obstacle_1"] etc.
                #         if any(n.lower() in object_name.lower() for n in self._debug_cuboid_names):
                #             self._debug_dump_cuboid_consistency(
                #                 object_name=object_name,
                #                 object_path=object_path,
                #                 lab_pos=current_pos.detach(),
                #                 lab_quat=current_quat.detach(),
                #             )
                #     except Exception as e:
                #         self.logger.debug(f"[CUBOID_DUMP] {object_name}: failed: {e}")
                # # -------------------------------------------------------------------------
                # # DEBUG: Compare IsaacLab pose vs cuRobo world-model pose (after update)
                #  # -------------------------------------------------------------------------
                # if do_pose_diff_log:
                #     try:
                #         cu_pose = self.get_object_pose(object_name)
                #         if cu_pose is None:
                #             self.logger.warning(
                #                 f"[POSEDIFF] {object_name}: cu_pose=None (mapping ok, but pose missing)"
                #             )
                #         else:
                #            # IsaacLab (env-local) pose
                #             lab_pos = current_pos.detach()
                #             lab_quat = current_quat.detach()  # (w,x,y,z)

                #             # cuRobo world-model pose
                #             cu_pos = cu_pose.position.detach()
                #             # Pose usually has .quaternion, but be robust
                #             if hasattr(cu_pose, "quaternion") and cu_pose.quaternion is not None:
                #                 cu_quat = cu_pose.quaternion.detach()
                #             else:
                #                 # fallback: try get_quaternion() if available
                #                 cu_quat = cu_pose.get_quaternion().detach()  # type: ignore

                #             # Position diff (meters)
                #             pos_diff = torch.linalg.vector_norm(lab_pos - cu_pos).item()

                #             # Rotation diff: angle = 2*acos(|dot(q1,q2)|)
                #             # Normalize to be safe
                #             lab_q = lab_quat / (torch.linalg.vector_norm(lab_quat) + 1e-12)
                #             cu_q = cu_quat / (torch.linalg.vector_norm(cu_quat) + 1e-12)
                #             dot = torch.sum(lab_q * cu_q).abs().clamp(0.0, 1.0).item()
                #             rot_rad = 2.0 * math.acos(dot)
                #             rot_deg = rot_rad * 180.0 / math.pi

                #             # Log level based on thresholds
                #             if (pos_diff > pos_warn_m) or (rot_deg > rot_warn_deg):
                #                 self.logger.warning(
                #                     f"[POSEDIFF][WARN] {object_name} "
                #                     f"pos_diff={pos_diff:.4f}m rot_diff={rot_deg:.2f}deg "
                #                     f"(lab_pos={lab_pos.tolist()}, cu_pos={cu_pos.tolist()})"
                #                 )
                #             else:
                #                 self.logger.debug(
                #                     f"[POSEDIFF] {object_name} "
                #                     f"pos_diff={pos_diff:.4f}m rot_diff={rot_deg:.2f}deg"
                #                 )
                #     except Exception as e:
                #         self.logger.debug(f"[POSEDIFF] {object_name}: failed to compute diff: {e}")
        ...
        if updated_count > 0:
            ...
            for object_name, object_path in object_mappings.items():
                ...
                p_w = obj.data.root_pos_w[self.env_id, :3]
                q_w = obj.data.root_quat_w[self.env_id, :4]
                ...
                self.motion_gen.world_coll_checker.update_obstacle_pose(  # type: ignore
                    object_path, curobo_pose, update_cpu_reference=False
                )
    ...
    # =====================================================================================
    # FRAME CONVERSION: World -> RobotPrim (reference_prim_path = /.../Robot)
    # =====================================================================================
    def _world_to_robotprim_pose(
        self,
        p_w: torch.Tensor,
        q_w_wxyz: torch.Tensor,
        *,
        env_id: int,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Convert a pose expressed in IsaacLab WORLD to the RobotPrim frame (reference prim).

        RobotPrim world pose is taken from:
          - robot.data.root_pos_w[env_id]
          - robot.data.root_quat_w[env_id]  (wxyz)
        ...
        Returns:
          (p_rel, q_rel_wxyz) on the same device/dtype as inputs (best-effort).
        """
        # Robot root pose in WORLD
        pR_w = self.robot.data.root_pos_w[env_id, :3]
        qR_w = self.robot.data.root_quat_w[env_id, :4]  # (wxyz)

        # Ensure same device for math
        device = p_w.device
        dtype = p_w.dtype
        pR_w = pR_w.to(device=device, dtype=dtype)
        qR_w = qR_w.to(device=device, dtype=dtype)
        q_w_wxyz = q_w_wxyz.to(device=device, dtype=dtype)

        # World rotation matrices
        R_R = PoseUtils.matrix_from_quat(qR_w)      # (3,3)
        R_w = PoseUtils.matrix_from_quat(q_w_wxyz)  # (3,3)

        # World -> RobotPrim
        p_rel = R_R.transpose(-1, -2) @ (p_w[:3] - pR_w)
        R_rel = R_R.transpose(-1, -2) @ R_w
        q_rel = PoseUtils.quat_from_matrix(R_rel)   # (wxyz)
        return p_rel, q_rel
    ...
    def _get_current_joint_state_for_curobo(self) -> JointState:
        ...
        # Convert only the joint-name metadata from Isaac Lab FR3 names to
        # the Panda names expected by the cuRobo Franka model. Joint values
        # remain unchanged.
        curobo_input_joint_names = [
            self.FR3_TO_PANDA_JOINT_NAMES.get(name, name)
        ...
        cu_js: JointState = JointState(
            ...
            joint_names=curobo_input_joint_names,
            tensor_args=self.tensor_args,
        )
    ...
    def _plan_to_contact_pose(
        ...
        try:
            ...
            if result.success.item():
                ...
                #     print(f"Optimized plan has {len(self._current_plan.position)} waypoints")
                ...
                #     print(f"Interpolated plan has {len(self._current_plan.position)} waypoints")
                ...
                # Keep Panda names while the plan remains inside cuRobo.
                # _plan_to_contact() may use the final state of this segment
                # as the start state of another planning phase.
                ...
                self._current_plan = (
                    self._current_plan.get_ordered_joint_state(
                        panda_arm_joint_names
                    )
                )
    ...
    def _plan_to_contact(
        ...
        # Planning is now completely finished. The numerical order of Panda
        # joints 1--7 is the same as the FR3 action order, so only rename the
        # metadata before returning the trajectory to Isaac Lab.
        panda_arm_joint_names = [
            "panda_joint1",
            "panda_joint2",
            "panda_joint3",
        ...
        self._current_plan.joint_names = fr3_arm_joint_names
    ...
    def _create_sphere_config(self, sphere_idx: int, sphere, robot_link_count: int) -> dict:
        ...
        return {
            ...
            "cfg": MeshSphereCfg(
                ...
                visual_material=PreviewSurfaceCfg(diffuse_color=color, opacity=1.0, emissive_color=color),
            ),
        }
    ...
    def update_world_and_plan_motion(
        ...
        # # ---------------------------------------------------------------------
        # # DEBUG: env_id consistency check
        # # ---------------------------------------------------------------------
        # if env_id != self.env_id:
        #     self.logger.warning(
        #         f"[ENV_ID_MISMATCH] update_world_and_plan_motion got env_id={env_id} "
        #         f"but planner was constructed with self.env_id={self.env_id}. "
        #         "Many internal methods use self.env_id; this can corrupt world sync / planning in multi-env."
        #     )
        ...
        # # ---------------------------------------------------------------------
        # # DEBUG: print goal pose (env-local) and current EE pose (env-local)
        # # ---------------------------------------------------------------------
        # try:
        #     tgt_cuda = self._to_curobo_device(target_pose)
        #     tgt_pos, tgt_rot = PoseUtils.unmake_pose(tgt_cuda)
        #     tgt_quat = PoseUtils.quat_from_matrix(tgt_rot)  # (w,x,y,z)
        #     self.logger.debug(
        #         f"[GOAL_POSE] env_id(arg)={env_id} planner_env_id={self.env_id} "
        #         f"pos={tgt_pos.detach().cpu().tolist()} quat(wxyz)={tgt_quat.detach().cpu().tolist()}"
        #     )
        # except Exception as e:
        #     self.logger.debug(f"[GOAL_POSE] failed to decode target_pose: {e}")

        # # Current EE (from robot state) before update_world()
        # try:
        #     js0 = self._get_current_joint_state_for_curobo()
        #     ee0 = self.get_ee_pose(js0)
        #     self.logger.debug(
        #         f"[EE_BEFORE] pos={ee0.position.detach().cpu().tolist()} "
        #         f"quat(wxyz)={ee0.quaternion.detach().cpu().tolist() if hasattr(ee0,'quaternion') else 'N/A'}"
        #     )
        # except Exception as e:
        #     self.logger.debug(f"[EE_BEFORE] failed: {e}")

        self.update_world()

        # # Current EE after update_world() (should be same, but helps catch device/order bugs)
        # try:
        #     js1 = self._get_current_joint_state_for_curobo()
        #     ee1 = self.get_ee_pose(js1)
        #     self.logger.debug(
        #         f"[EE_AFTER]  pos={ee1.position.detach().cpu().tolist()} "
        #         f"quat(wxyz)={ee1.quaternion.detach().cpu().tolist() if hasattr(ee1,'quaternion') else 'N/A'}"
        #     )
        # except Exception as e:
        #     self.logger.debug(f"[EE_AFTER] failed: {e}")
    ...
    # =====================================================================================
    # WORLD COLLISION QUERY WRAPPERS (cuRobo API-correct; frame conversion done externally)
    # =====================================================================================

    def _ensure_collision_query_buffer(
        self,
        n_spheres: int,
        batch: int = 1,
        horizon: int = 1,
        ...
        """
        Ensure CollisionQueryBuffer is initialized with shape (B,H,Ns,4).
        Re-allocates only if shape changes.

        Notes:
          - WorldMeshCollision.get_sphere_distance/collision require collision_query_buffer, weight,
            activation_distance, and query_sphere shaped [B,H,Ns,4].
        """
        if not hasattr(self, "_collision_query_buffer") or self._collision_query_buffer is None:
            self._collision_query_buffer = CollisionQueryBuffer()

        shape = (batch, horizon, n_spheres, 4)
        wcc = self.motion_gen.world_coll_checker
    ...
    def _pack_spheres_from_q_robot_frame(self, q: torch.Tensor) -> torch.Tensor:
        """
        q -> spheres in *robot/kinematics frame* (NOT world transformed here).
        Returns: (B,1,Ns,4) with [x,y,z,r] in kinematics frame.

        The caller (gen_data_multi_obs.py) is responsible for transforming xyz into Isaac world
        (or whatever "cuRobo world" you want to compare against PhysX).
        """
        if q.dim() == 1:
            q = q.unsqueeze(0)
        q = q.to(device=self.tensor_args.device, dtype=self.tensor_args.dtype)

        sphere_batches = self.motion_gen.kinematics.get_robot_as_spheres(q)
        # sphere_batches is typically length B; each element is iterable of Sphere objects.

        rows_b = []
        for b in range(q.shape[0]):
            rows = []
            for s in list(sphere_batches[b]):
                pos = s.position
                if not isinstance(pos, torch.Tensor):
                    pos = torch.tensor(pos, device=self.tensor_args.device, dtype=self.tensor_args.dtype)
                else:
                    pos = pos.to(device=self.tensor_args.device, dtype=self.tensor_args.dtype)

                r = float(s.radius)
                r_t = torch.tensor([r], device=pos.device, dtype=pos.dtype)
                rows.append(torch.cat([pos.reshape(3), r_t], dim=0))  # (4,)

            if len(rows) == 0:
                # Unusual but keep API safe
                rows_b.append(torch.zeros((0, 4), device=self.tensor_args.device, dtype=self.tensor_args.dtype))
            else:
                rows_b.append(torch.stack(rows, dim=0))  # (Ns,4)

        spheres = torch.stack(rows_b, dim=0).unsqueeze(1)  # (B,1,Ns,4)
        return spheres

    def probe_collision_from_spheres(
        self,
        spheres_B1Ns4: torch.Tensor,
        *,
        env_id: int = 0,
        ...
        """
        Correct wrapper for WorldMeshCollision.get_sphere_distance() using required arguments.

        Args:
            spheres_B1Ns4: (B,1,Ns,4) query spheres [x,y,z,r].
                          IMPORTANT: xyz must already be expressed in the SAME frame as cuRobo world.
                          (You said you'll handle this in gen_data_multi_obs.py.)
            env_id: environment index (used as env_query_idx tensor).
            activation_distance: activation distance tensor (scalar).
            weight: weight tensor (scalar).
            compute_esdf: if True, returns ESDF where inside obstacle is positive and outside is negative.

        Returns:
            dict with keys:
              - in_collision (bool)
              - max_penetration (float)  # >0 means penetration depth in ESDF convention
              - min_distance_outside (float)  # most negative value (farthest outside among spheres)
              - raw_min (float), raw_max (float)  # min/max of returned tensor
              - details (str)
        """
        wcc = self.motion_gen.world_coll_checker

        if spheres_B1Ns4.dim() != 4 or spheres_B1Ns4.shape[-1] != 4:
            raise ValueError(f"spheres must be (B,1,Ns,4), got {tuple(spheres_B1Ns4.shape)}")

        spheres = spheres_B1Ns4.to(device=self.tensor_args.device, dtype=self.tensor_args.dtype)
        B, H, Ns, _ = spheres.shape
        if H != 1:
            # This wrapper is meant for single-step probes; expand if you want trajectory queries
            raise ValueError(f"Expected horizon H=1, got H={H}")

        # Ensure buffer matches (B,1,Ns,4)
        self._ensure_collision_query_buffer(n_spheres=Ns, batch=B, horizon=1)

        # Required tensors
        w_t = self.tensor_args.to_device([float(weight)])
        act_t = self.tensor_args.to_device([float(activation_distance)])

        # env_query_idx must be a torch tensor (optional in API, but we want deterministic)
        env_query_idx = torch.full(
            (B,),
            int(env_id),
        ...
        # Call the API with correct signature
        # d = self.collision_dis_checker(
        #     spheres,
        ...
        # d = self.collision_dis_checker(
        #     spheres,
        #     self.motion_gen.rollout_fn.primitive_collision_constraint._collision_query_buffer,
        #     self.motion_gen.rollout_fn.primitive_collision_constraint.weight,
        #     sweep_steps= self.motion_gen.rollout_fn.primitive_collision_constraint.sweep_steps,
        #     activation_distance= self.motion_gen.rollout_fn.primitive_collision_constraint.activation_distance,
        #     speed_dt= self.motion_gen.rollout_fn.primitive_collision_constraint.speed_dt,
        #     enable_speed_metric= self.motion_gen.rollout_fn.primitive_collision_constraint.use_speed_metric,
        #     env_query_idx=None,
        #     return_loss= self.motion_gen.rollout_fn.primitive_collision_constraint.return_loss,
        # )

        d = self.motion_gen.rollout_fn.primitive_collision_constraint.coll_check_fn(
            spheres,
            # self.motion_gen.rollout_fn.primitive_collision_constraint._collision_query_buffer,
            self._collision_query_buffer,
            self.motion_gen.rollout_fn.primitive_collision_constraint.weight,
            ...
            activation_distance=self.motion_gen.rollout_fn.primitive_collision_constraint.activation_distance,
            return_loss=self.motion_gen.rollout_fn.primitive_collision_constraint.return_loss,
            sum_collisions=self.motion_gen.rollout_fn.primitive_collision_constraint.sum_collisions,
        )

        coll_constraint = self.collision_detector(
            spheres, env_query_idx=None
            )
        

        # d is (B,1,Ns) or similar; be robust:
        if not isinstance(d, torch.Tensor):
            raise RuntimeError(f"Expected tensor output from get_sphere_distance, got {type(d)}")

        d_min = float(d.min().item()) if d.numel() > 0 else 0.0
        d_max = float(d.max().item()) if d.numel() > 0 else 0.0

        # ESDF convention (per docstring): inside obstacle positive, outside negative.
        # Collision iff any sphere is inside => max(d) > 0
        # in_col = bool(d_max > 0.0)

        # dに１つでも正の値があれば衝突とみなす（ESDF convention）
        in_col = bool((d > 0.0).any().item())

        return {
            "in_collision": in_col,
            # "max_penetration": d_max,         # penetration depth proxy (>0 means inside)
            # "min_distance_outside": d_min,    # most negative
            # "raw_min": d_min,
            # "raw_max": d_max,
            # "d":d,  # Include raw tensor for debugging
            "d_sum": float(d.sum().item()) if d.numel() > 0 else 0.0,  # Total collision cost (sum of all spheres)
            # "d.shape": d.shape,
            # "details": "WorldMeshCollision.get_sphere_distance(compute_esdf={})".format(compute_esdf),
            # "coll_constraint": coll_constraint,  # Add raw collision constraint output for debugging
        }

    def probe_collision_from_q(
        self,
        q: torch.Tensor,
        *,
        env_id: int = 0,
        ...
        """
        Convenience wrapper: q -> spheres (robot frame) -> probe.
        NOTE: This does NOT do frame conversion. If you need world alignment, call:
          spheres = _pack_spheres_from_q_robot_frame(q)
          spheres = (gen_data_multi_obs.pyでworldに変換)
          probe_collision_from_spheres(spheres, ...)
        """
        spheres = self._pack_spheres_from_q_robot_frame(q)  # (B,1,Ns,4) in robot frame
        return self.probe_collision_from_spheres(
            spheres,
            env_id=env_id,
    ...
    def _quat_wxyz_from_rotmat(self, R: torch.Tensor) -> torch.Tensor:
        """R: (3,3) -> q(wxyz)"""
        # robust enough for debug
        tr = R.trace()
        if tr > 0:
    ...
    def _quat_angle_deg(self, q1_wxyz: torch.Tensor, q2_wxyz: torch.Tensor) -> float:
        """geodesic angle between quats, in degrees"""
        q1 = q1_wxyz / (torch.linalg.vector_norm(q1_wxyz) + 1e-12)
        q2 = q2_wxyz / (torch.linalg.vector_norm(q2_wxyz) + 1e-12)
        dot = torch.sum(q1 * q2).abs().clamp(0.0, 1.0).item()
        ang = 2.0 * math.acos(dot)
        return ang * 180.0 / math.pi
    def debug_check_robot_root_matches_robot_prim(self, env, robot, usd_helper, env_id: int):
        """
        Compare:
        - USD prim /World/envs/env_{env_id}/Robot world pose
        - robot.data.root_pos_w/root_quat_w[env_id]
        """
        ...
        # USD: LocalToWorld 4x4
        from pxr import UsdGeom, Usd
        xform = UsdGeom.Xformable(prim)

        # Use Default timecode (recommended for xform cache semantics)
        M = xform.ComputeLocalToWorldTransform(Usd.TimeCode.Default())  # Gf.Matrix4d

        # -----------------------------
        # Correct extraction (IMPORTANT)
        # -----------------------------
        # Translation (world)
        t = M.ExtractTranslation()  # Gf.Vec3d
        p_usd = torch.tensor([t[0], t[1], t[2]], dtype=torch.float64)

        # Rotation (world): returns Gf.Quatd (real, imag)
        # real = w, imag = (x,y,z)
        q = M.ExtractRotationQuat()  # Gf.Quatd
        qw = float(q.GetReal())
        qx, qy, qz = q.GetImaginary()
        q_usd = torch.tensor([qw, float(qx), float(qy), float(qz)], dtype=torch.float64)
        q_usd = q_usd / (torch.linalg.vector_norm(q_usd) + 1e-12)

        # IsaacLab: robot root
        p_lab = robot.data.root_pos_w[env_id, :3].detach().cpu().to(torch.float64)
        q_lab = robot.data.root_quat_w[env_id, :4].detach().cpu().to(torch.float64)  # (wxyz)
        q_lab = q_lab / (torch.linalg.vector_norm(q_lab) + 1e-12)

        pos_diff = torch.linalg.vector_norm(p_lab - p_usd).item()
        rot_diff_deg = self._quat_angle_deg(q_lab, q_usd)

        print(f"[CHECK] prim={prim_path}")
        print(f"  USD  pos={p_usd.tolist()}  quat(wxyz)={q_usd.tolist()}")
        print(f"  LAB  pos={p_lab.tolist()}  quat(wxyz)={q_lab.tolist()}")
        print(f"  DIFF pos={pos_diff:.6f} m, rot={rot_diff_deg:.6f} deg")

        d = (p_lab - p_usd)
        print("diff vec [m] =", d.tolist())
        print("norm =", float(torch.linalg.vector_norm(d)))

        env_origin = env.scene.env_origins[env_id].detach().cpu().to(torch.float64)
        print("env_origin =", env_origin.tolist())
        print("diff - env_origin =", (d - env_origin).tolist())
        print("diff + env_origin =", (d + env_origin).tolist())
